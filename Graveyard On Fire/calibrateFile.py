...
img = cv.imread('calibrate061.jpg')
h, w = img.shape[:2]
...
#inches
realLeft = np.array([-24.277, 52.37, 0])
realTop = np.array([6.896, 77.033, 0])
realRight = np.array([26.625, 52, 0])
realBottom = np.array([-4.5, 27.375, 0])
...
convertImg = cv.imread('linepoints.jpg')
...
"""
#saves to file
np.savez('cam06.npz', mtx=mtx, dist=dist, newcameramtx=newcameramtx, roi=roi)
